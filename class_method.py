...
def select_table(column_name, table_name, id):
    '''
...
def insert_table(table_name, v1, v2, v3, vt):
    '''
    ...
    with closing(psycopg2.connect(dbname='verbs',
                                  ...
                                  host='localhost')) as conn:
        with conn.cursor() as cursor:
            cursor.execute("INSERT INTO {} (one_form, two_form, three_form, translate)"
                           " VALUES ('{}','{}','{}','{}')".format(table_name, v1, v2, v3, vt))
            conn.commit()


def delete_val_table():
    '''
    ...
    with closing(psycopg2.connect(dbname='verbs',
                                  ...
                                  host='localhost')) as conn:
        with conn.cursor() as cursor:
            cursor.execute("DELETE FROM verbs_form WHERE id=9")
            conn.commit()


len__tb = len_table('verbs_form')
for i in range(1, len__tb+1):
    a = select_table('one_form', 'verbs_form', i)
    two = select_table('two_form', 'verbs_form', i)
    three = select_table('three_form', 'verbs_form', i)
    trans = select_table('translate', 'verbs_form', i)
    print(a)
