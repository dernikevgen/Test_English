...
class Base(QMainWindow, ConnectDjangoApi):
    ...
    def message_box(self):
        ...
        hello.setFixedSize(500, 250)
        lable_help = QLabel('Приложению необходим доступ в интернет', hello)
        lable_help.move(50, 100)
        because = QPushButton('Хорошо, спасибо', hello)
        because.resize(self.btn_assert.sizeHint())
        because.adjustSize()
        because.setGeometry(150, 150, 200, 30)
        because.clicked.connect(hello.close)
    ...
    def start_program(self):
        ...
        self.true_limit_2.setText(f'{self.true_input} из'
                                  f' {self.max_flags}')
        self.lbl_limit.setText(f'{self.id} / {self.max_val}')
        self.true_limit.adjustSize()
        self.true_limit_2.adjustSize()
        self.lbl_lim_set_stat.adjustSize()
        self.true_limit.move(620, 280)
    ...
    def choice_verbs(self):
        ...
        '''
        self.one = self.verbs_dict[self.print_one_key[self.key_db]]
        self.two = self.verbs_dict[self.print_one_key[self.key_db]+1]
        self.three = self.verbs_dict[self.print_one_key[self.key_db]+2]
        self.trans = self.verbs_dict[self.print_one_key[self.key_db]+3]
    ...
    def next_value(self):
        ...
        if self.id == self.max_val-1:
            ...
            self.choice_verbs()
            self.btn_next.setFocus()
            self.lbl_limit.setText(f'{self.id} /'
                                   f' {self.max_val}')
            self.line_1.setText(self.one)
            self.line_1_2.setText(self.one)
            self.line_2.setReadOnly(False)
